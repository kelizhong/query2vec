...
def embedding_rnn_decoder(decoder_inputs,
  ...
  """
  # Nick modified
  with variable_scope.variable_scope("RNN/EmbeddingWrapper", reuse=True) as curr_scope:
...
def model_with_buckets(encoder_inputs, decoder_inputs, targets, weights,
  ...
  with ops.name_scope(name, "model_with_buckets", all_inputs):
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(variable_scope.get_variable_scope(),
        ...
        bucket_outputs, last_state, bucket_states = seq2seq(encoder_inputs[:bucket[0]],
                                                            decoder_inputs[:bucket[1]])
        outputs.append(bucket_outputs)
        states.append(bucket_states[-1])
        last_states.append(last_state[0])
        if per_example_loss:
  ...
  return outputs, losses, states, last_states
